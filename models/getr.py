...
class GETR(nn.Module):
    ...
    def __init__(self, rgb_backbone, canny_backbone, gabor_backbone, transformer):
        ...
        self.canny_input_proj = nn.Conv2d(canny_backbone.num_channels, hidden_dim, kernel_size=1)
        self.gabor_input_proj = nn.Conv2d(gabor_backbone.num_channels, hidden_dim, kernel_size=1)
        self.rgb_backbone = rgb_backbone
        self.canny_backbone = canny_backbone
        self.gabor_backbone = gabor_backbone
        self.mask_head = MaskHeadSmallConv(hidden_dim * 2, [2048, 1024, 512, 256], hidden_dim)
        # self.fusion_embedding = nn.Embedding(, hidden_dim)

    def forward(self, rgb: NestedTensor, canny: NestedTensor, gabor: NestedTensor, final_size, gate_flag: bool):

        if isinstance(rgb, (list, torch.Tensor)):
            rgb = nested_tensor_from_tensor_list(rgb)

        if isinstance(canny, (list, torch.Tensor)):
            canny = nested_tensor_from_tensor_list(canny)
        if isinstance(gabor, (list, torch.Tensor)):
            gabor = nested_tensor_from_tensor_list(gabor)

        rgb_features, rgb_pos = self.rgb_backbone(rgb)
        canny_features, canny_pos = self.canny_backbone(canny)
        gabor_features, gabor_pos = self.gabor_backbone(gabor)

        rgb_src, rgb_mask = rgb_features[-1].decompose()
        canny_src, canny_mask = canny_features[-1].decompose()
        gabor_src, gabor_mask = gabor_features[-1].decompose()

        assert rgb_mask is not None
        assert canny_mask is not None
        assert gabor_mask is not None

        rgb_proj = self.rgb_input_proj(rgb_src)
        canny_proj = self.canny_input_proj(canny_src)
        gabor_proj = self.gabor_input_proj(gabor_src)
        fusion_memory = self.transformer(rgb_proj, rgb_mask, rgb_pos[-1], canny_proj, canny_mask,
                                         canny_pos[-1], gabor_proj, gabor_mask,
                                         gabor_pos[-1], gate_flag)
        out = {}
...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, context_dim):
        ...
        self.canny_adapter1 = nn.Sequential(nn.Conv2d(fpn_dims[0], inter_dims[1], 1),
                                            nn.BatchNorm2d(inter_dims[1]),
                                            nn.ReLU(inplace=True))
        self.canny_adapter2 = nn.Sequential(nn.Conv2d(fpn_dims[1], inter_dims[1], 1),
                                            nn.BatchNorm2d(inter_dims[1]),
                                            nn.ReLU(inplace=True))
        self.canny_adapter3 = nn.Sequential(nn.Conv2d(fpn_dims[2], inter_dims[2], 1),
                                            nn.BatchNorm2d(inter_dims[2]),
                                            nn.ReLU(inplace=True))
        self.canny_adapter4 = nn.Sequential(nn.Conv2d(fpn_dims[3], inter_dims[3], 1),
                                            nn.BatchNorm2d(inter_dims[3]),
                                            nn.ReLU(inplace=True))

        self.gabor_adapter1 = nn.Sequential(nn.Conv2d(fpn_dims[0], inter_dims[1], 1),
                                            nn.BatchNorm2d(inter_dims[1]),
                                            nn.ReLU(inplace=True))
        self.gabor_adapter2 = nn.Sequential(nn.Conv2d(fpn_dims[1], inter_dims[1], 1),
                                            nn.BatchNorm2d(inter_dims[1]),
                                            nn.ReLU(inplace=True))
        self.gabor_adapter3 = nn.Sequential(nn.Conv2d(fpn_dims[2], inter_dims[2], 1),
                                            nn.BatchNorm2d(inter_dims[2]),
                                            nn.ReLU(inplace=True))
        self.gabor_adapter4 = nn.Sequential(nn.Conv2d(fpn_dims[3], inter_dims[3], 1),
                                            nn.BatchNorm2d(inter_dims[3]),
                                            nn.ReLU(inplace=True))
...
def build(args):
    ...
    canny_backbone = build_backbone(args)
    gabor_backbone = build_backbone(args)
