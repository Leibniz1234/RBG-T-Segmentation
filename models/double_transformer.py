...
class Transformer(nn.Module):
    ...
    def forward(self, rgb_src, rgb_mask, rgb_pos_embed, canny_src, canny_mask, canny_pos_embed, gabor_src, gabor_mask,
                gabor_pos_embed, gate_flag):
        # flatten NxCxHxW to HWxNxC

        bs, c, h, w = rgb_src.shape

        fusion_pos_embed = self.embedding_generator(
            NestedTensor(torch.cat([rgb_src, canny_src, gabor_src], 2),
                         torch.cat([rgb_mask, canny_mask, gabor_mask], 1)))
        ...
        # temperature_branch
        canny_src = canny_src.flatten(2).permute(2, 0, 1)
        canny_pos_embed = canny_pos_embed.flatten(2).permute(2, 0, 1)
        canny_mask = canny_mask.flatten(1)

        # gabor_branch
        gabor_src = gabor_src.flatten(2).permute(2, 0, 1)
        gabor_pos_embed = gabor_pos_embed.flatten(2).permute(2, 0, 1)

        fusion_memory = self.fusion_encoder(rgb_src, canny_src, gabor_src, memory_key_padding_mask=rgb_mask,
                                            rgb_pos=rgb_pos_embed, canny_pos=canny_pos_embed, gabor_pos=gabor_pos_embed,
...
class FusionEncoder(nn.Module):
    def __init__(self, encoder_layer, Linear_layers, num_layers, norm=None):
        ...
        self.canny_layers = _get_clones(encoder_layer, num_layers)
        self.gabor_layers = _get_clones(encoder_layer, num_layers)
    ...
    def forward(self, rgb_memory, canny_memory, gabor_memory,
                tgt_mask: Optional[Tensor] = None,
                memory_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None,
                rgb_pos: Optional[Tensor] = None,
                canny_pos: Optional[Tensor] = None,
                gabor_pos: Optional[Tensor] = None,
                fusion_pos: Optional[Tensor] = None,
                gate_flag: bool = True):

        s, b, c = rgb_memory.shape

        fusion_key_padding_mask = torch.cat([memory_key_padding_mask, memory_key_padding_mask,memory_key_padding_mask]
                                            , 1)
        for i in range(self.num_layers):
            ...
            canny_memory = self.canny_layers[i](canny_memory, src_mask=memory_mask,
                                                src_key_padding_mask=memory_key_padding_mask,
                                                pos=canny_pos)
            gabor_memory = self.gabor_layers[i](gabor_memory, src_mask=memory_mask,
                                                src_key_padding_mask=memory_key_padding_mask,
                                                pos=gabor_pos)

            fusion_memory = torch.cat([rgb_memory, canny_memory, gabor_memory], 0)
            ...
            if gate_flag:
                ...
                canny_memory = canny_memory + fusion_memory[s:2*s] * attention_weights[s:2*s]
                gabor_memory = gabor_memory + fusion_memory[2*s:] * attention_weights[2*s:]
            else:
                rgb_memory = rgb_memory
                canny_memory = canny_memory

        if gate_flag:
            output = fusion_memory[:s] * attention_weights[:s] + fusion_memory[s:2*s] * attention_weights[s:2*s]\
                     + fusion_memory[2*s:] * attention_weights[2*s:]
        else:
            output = rgb_memory
