...
class RGBT:
    ...
    def __getitem__(self, idx):
        ...
        # canny
        blurred = cv2.GaussianBlur(np.array(img), (3, 3), 3)
        canny = cv2.Canny(blurred, 60, 40)
        canny = (canny - np.min(canny)) / (np.max(canny) - np.min(canny)) * 255
        ...
        gabor = cv2.filter2D(rgb_image, cv2.CV_8UC3, kernel)
        gabor = Image.fromarray(gabor).convert('RGB')
        ...
        if self.transforms is not None:
            img, canny, gabor, target = self.transforms(img, canny, gabor, target)
        return img, canny, gabor, target, img_path
